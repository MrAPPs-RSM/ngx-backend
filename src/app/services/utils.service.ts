...
export class UtilsService {
  ...
  public static parseParams(filters: any, data: any): any {
    // Parse other params
    for (const u in filters.where) {
      if (u === 'and' || u === 'or') {
        for (const c in filters.where[u]) {
          if (Object.values(filters.where[u][c])[0][0] === ':' && Object.values(filters.where[u][c])[0] !== ':id') {
            const key: string = (Object.values(filters[u][c])[0] as string).replace(':', '');
            const filterKey: string = Object.keys(filters.where[u][c])[0];
        ...
        if (filters.where[u][0] === ':' && filters.where[u] !== ':id') {
          const key: string = filters.where[u].replace(':', '');
    ...
    for (let i = 0; i < end.length; i++) {
      ...
      if (end[i] === ':') {
        ...
        endpoint = endpoint.replace(
          end.substring(i, end.indexOf('/', i)),
          encodeURIComponent(data[end.substring(i, end.indexOf('/', i)).replace(':', '')])
        );
      }
    }
  ...
  public static checkSingleConditionalValue(rawValue: any, conditionalValue: ConditionalValue): boolean {
    const operator = conditionalValue.operator ?? 'eq';
    let result: boolean;
    switch (operator) {
      case 'neq':
        result = rawValue !== conditionalValue.value;
        break;
      case 'eq':
        result = rawValue === conditionalValue.value;
        break;
